...
public class LogActivity extends Activity {
	...
	private void setConnections() {
		...
		logAdapter = new LogAdapter(this, R.layout.list_item_pft_entry,
				notesList);
	...
	private void setOnClickListeners() {
			...
			listView.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View v, int row,
						long arg3) {
					Intent intent = null;
					if(notesList.get(row).getName().equals("PFT")){
						intent = new Intent(v.getContext(), PftLogViewActivity.class);
					}else if(notesList.get(row).getName().equals("CFT")){
						intent = new Intent(v.getContext(), CftLogViewActivity.class);
					}
					else{
	...
	private void createLogDialog(){
		...
		alert.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if(which == 0){
					Intent intent = new Intent(LogActivity.this.getBaseContext(), PftLogViewActivity.class);
					intent.putExtra("list", notesList);
					...
					Intent intent = new Intent(LogActivity.this.getBaseContext(), CftLogViewActivity.class);
					intent.putExtra("list", notesList);
					intent.putExtra("data", mData);
					intent.putExtra("index", -1);
					startActivityForResult(intent, LOGVIEW);
